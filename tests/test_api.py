...
def test_create_and_connect_bundle_schema(client, monkeypatch):
    ...
    body = create.json()
    assert body["tenant_id"] == "org_123"
    assert body["instance_id"] == "morphvm_test123"
    assert seen["auth_header"] == "Bearer user-morph-key-abc"
    assert seen["tunnel_auth_header"] == "Bearer user-morph-key-abc"
    assert seen["provision_auth_header"] == "Bearer user-morph-key-abc"
...
def test_snapshot_and_restore_updates_instance_id(client, monkeypatch):
    ...
    assert seen["delete_auth_header"] == "Bearer user-morph-key-abc"
